...
class Logout_View(View):
    ...
    def get(self,request):
        logout(self.request)
        return redirect ('management:dashboard',permanent=True)
...
def deductions(request):
    deductions=Deductions.objects.all().order_by('-amount')
    return render(request,'management/employee/deductions.html',{'deductions':deductions,})

def advance(request):
    advances=Cashadvance.objects.all().order_by('-date')
    return render(request,'management/employee/Advance.html',{'advances':advances})